# Contributing to aw-watcher-lid
...
```bash
make dev    # editable install with dev dependencies, installs pre-commit hooks
make test
make lint
```
...
- `fix: ignore lid events shorter than the configured threshold`
- `feat: add new check`
- `docs: update README`

Rather than:

- `This commit fixes the lid event handling`
- `Added new check`
